...
class Field_Parser3D:
    ...
    def getLimits(self, filename):
        ...
        for line in infile.readlines():
            ...
            value = list(map(float, splitline))
    ...
    def checkLimits(self, arrayLimits, value):
        if value[0] >= arrayLimits[0] and value[0] <= arrayLimits[1]:
            if value[1] >= arrayLimits[2] and value[1] <= arrayLimits[3]:
                if value[2] >= arrayLimits[4] and value[2] <= arrayLimits[5]:
                    return True
        else:
            return False
    ...
    def checkGrid(self, step, value):
        ...
        list(map(int, localStep))
        list(map(int, localValue))
    ...
    def parse(self, filename, xmin, xmax, ymin, ymax, zmin, zmax, xstep, ystep, zstep):
        ...
        for line in infile1.readlines():
            splitLine = line.split()
            rawNumbers = list(map(float, splitLine))
            #           Maps data points to integers so that they can be evaluated for stepsize
            testRS = list(map(int, rawNumbers))

            if self.checkGrid(step, rawNumbers) and self.checkLimits(usrLimits, rawNumbers):
