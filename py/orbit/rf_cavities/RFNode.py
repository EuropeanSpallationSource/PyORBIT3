...
class Frequency_RFNode(Base_RFNode):
    def __init__(self, RFFreq, RFE0TL, RFPhase, length, name="frequency_rfnode"):
        """
...
class Harmonic_RFNode(Base_RFNode):
    def __init__(self, ZtoPhi, dESync, RFHNum, RFVoltage, RFPhase, length, name="harmonic_rfnode"):
        """
        Constructor. Creates Harmonic
        RF Cavity TEAPOT element
        """
        Base_RFNode.__init__(self, length, name)
        self.harmonicnode = Harmonic_Cav(ZtoPhi, dESync, RFHNum, RFVoltage, RFPhase)
...
class Dual_Harmonic_RFNode(Base_RFNode):
    def __init__(
        ...
        """
        Base_RFNode.__init__(self, length, name)
        self.dualharmonicnode = Dual_Harmonic_Cav(ZtoPhi, RFHNum, RatioRFHNum, RFVoltage, RatioVoltage, RFPhase, RFPhase2)
...
class BRhoDep_Harmonic_RFNode(Base_RFNode):
    def __init__(self, ZtoPhi, accelDict, bunch, length, name="brho_timedep_harmonic_rfnode"):
        ...
        eold = keold + mass
        RFVoltage = interp(time, n_tuple, time_tuple, RFVoltage_tuple)
        RFPhase = interp(time, n_tuple, time_tuple, RFPhase_tuple)
        BRho = interp(time, n_tuple, time_tuple, BRho_tuple)
        pcnew = 0.299792458 * charge * BRho
        enew = math.sqrt(pcnew * pcnew + mass * mass)
        kenew = enew - mass
        bunch.getSyncParticle().kinEnergy(kenew)
        dESync = enew - eold
        Zsync = syncZ(ZtoPhi, gammaTrans, gamma, charge, dESync, RFHNum, RFVoltage, RFPhase)
...
class SyncPhaseDep_Harmonic_RFNode(Base_RFNode):
    def __init__(self, ZtoPhi, accelDict, bunch, length, name="syncphase_timedep_harmonic_rfnode"):
        """
        ...
        RFVoltage = interp(time, n_tuple, time_tuple, RFVoltage_tuple)
        RFPhase = interp(time, n_tuple, time_tuple, RFPhase_tuple)
        SyncPhase = interp(time, n_tuple, time_tuple, SyncPhase_tuple)
        dESync = charge * RFVoltage * math.sin(math.pi * (RFHNum * SyncPhase + RFPhase) / 180.0)
        kenew = keold + dESync
        bunch.getSyncParticle().kinEnergy(kenew)
...
class Barrier_RFNode(Base_RFNode):
    def __init__(
        ...
        """
        Base_RFNode.__init__(self, length, name)
        self.barriernode = Barrier_Cav(ZtoPhi, RFVoltage, RFPhasep, RFPhasem, dRFPhasep, dRFPhasem)
...
class TimeDep_Barrier_RFNode(Base_RFNode):
    def __init__(self, ZtoPhi, accelDict, bunch, length, name="syncphase_timedep_harmonic_rfnode"):
        ...
        time = bunch.getSyncParticle().time()
        RFVoltage = interp(time, n_tuple, time_tuple, RFVoltage_tuple)
        RFPhasep = interp(time, n_tuple, time_tuple, RFPhasep_tuple)
        RFPhasem = interp(time, n_tuple, time_tuple, RFPhasem_tuple)
        dRFPhasep = interp(time, n_tuple, time_tuple, dRFPhasep_tuple)
        dRFPhasem = interp(time, n_tuple, time_tuple, dRFPhasem_tuple)
        self.barriernode = Barrier_Cav(ZtoPhi, RFVoltage, RFPhasep, RFPhasem, dRFPhasep, dRFPhasem)
        self.setType("barrier rf node")
        self.setLength(0.0)
...
def syncZ(ZtoPhi, gammaTrans, gamma, charge, dESync, RFHNum, RFVoltage, RFPhase):
    """
