...
class TrialPoint:
    ...
    def addVariableProxy(self, variableProxy):
        if variableProxy.getName() in self._varProxy_dict:
    ...
    def __init__(self, *arg, **kwargs):
        ...
        if not (len(arg) != 1 and len(arg) != 2 and len(arg) != 3):
            if len(arg) == 1:
                ...
                self.value = arg[1]
                self.step = arg[2]
        elif len(arg) == 0 and len(kwargs) == 3 and ("name" in kwargs) and ("value" in kwargs) and ("step" in kwargs):
...
class SearchAgorithm(NamedObject):
    ...
    def init(self):
        if self.initTrialPoint == None or self.solver == None:
            return False
        return True
