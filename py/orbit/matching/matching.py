...
class Optics:
    ...
    def print_line(self):
        for j in range(0, len(self.line)):
            print(
                j,
                self.line[j].data["keyword"],
                "s:",
                self.line[j].data["s"],
                "L:",
                self.line[j].data["L"],
                360.0 * self.line[j].data["mux"],
                self.line[j].data["bety"],
                self.line[j].data["alfy"],
            )
...
class EnvelopeSolver:
    ...
    def func_odeint(self, y, s, emitx, emity, sigma_p, Ksc):
        ...
        f0 = y[1]
        f1 = (
            -(k1 / lj + (anglej / lj) ** 2) * y[0]
            + emitx**2 / y[0] ** 3
            + 0.5 * Ksc / (y[0] + y[2])
            + y[4] * sigma_p**2 * anglej / (y[0] * lj)
        )
        f2 = y[3]
    ...
    def Dfunc_odeint(self, y, s, emitx, emity, sigma_p, Ksc):
        ...
        a1 = (
            -(k1 / lj + (anglej / lj) ** 2) * y[1]
            - 3.0 * y[1] * emitx**2 / y[0] ** 4
            - 0.5 * Ksc * (y[1] + y[3]) / (y[0] + y[2]) ** 2
            + y[5] * sigma_p**2 * anglej / (y[0] * lj)
            - y[4] * y[1] * sigma_p**2 * anglej / (y[0] ** 2 * lj)
        )
        ...
        a5 = (
            -(k1 / lj + (anglej / lj) ** 2) * y[5]
            + 0.5 * Ksc / (y[0] * (y[0] + y[2])) * y[5]
            - 0.5 * Ksc / (y[0] * (y[0] + y[2])) ** 2 * y[4] * (y[1] * (y[0] + y[2]) + y[0] * (y[1] + y[3]))
        )
    ...
    def func_smooth(self, x, phase0x, phase0y, length, emitx, emity, Ksc):
        ...
        ky = (phase0y / length) ** 2
        return [
            emitx**2 / x[0] ** 3 - kx * x[0] + 0.5 * Ksc / (x[0] + x[1]),
            emity**2 / x[1] ** 3 - ky * x[1] + 0.5 * Ksc / (x[0] + x[1]),
        ]
