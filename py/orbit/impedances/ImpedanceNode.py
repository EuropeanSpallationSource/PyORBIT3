...
class LImpedance_Node(DriftTEAPOT):
    def __init__(self, phaseLength, nMacrosMin, nBins, name="LImpedance node"):
        """
...
class TImpedance_Node(DriftTEAPOT):
    def __init__(self, phaseLength, nMacrosMin, nBins, useX, useY, name="TImpedance node"):
        """
        Constructor. Creates TImpedance element.
        """
        DriftTEAPOT.__init__(self, name)
        self.timpedance = TImpedance(phaseLength, nMacrosMin, nBins, useX, useY)
        ...
        if self.useX != 0:
            ...
            for n in range(self.nBins // 2):
                ...
                if Freq_m < 0:
                    Freq_m = -Freq_m
                    sign_m = -1.0
                zp_mode = interp(Freq_p, self.freq_range, self.freq_tuple, z_tuple)
                zm_tmp = interp(Freq_m, self.freq_range, self.freq_tuple, z_tuple)
